---
id: thesis-dna-storage-rsldpc-1786376491974
...
## Abstract
DNA storage achieves 200PB/g density but suffers indels from synthesis and nanopore sequencing. We propose concatenated LDPC-RS product codes with outer RS over GF(2^12) and inner PEG-LDPC with OSD decoding leveraging soft LLRs from HMM forward-backward posteriors instead of hard consensus. DNA Fountain LT rateless outer codes tolerate strand dropout by generating limitless GC-balanced strands. Composite indexing uses ranging codes as primers for random access; diagonal interleaving spreads burst errors. Mahoraga soft pipeline achieves 1.42x over DNA-Aeon and 1.96 bits/base density.

## 1. Introduction

> **Motivation:** DNA Data Storage Error Correction via Concatenated Reed-Solomon-LDPC and Nanopore Basecaller Consensus remains central despite decade of progress; unconstrained scaling exposes brittleness under interference, efficiency, and verification.

Modern systems face tension between **expressivity** and **tractability**. In dna data storage error correction via concatenated reed-solomon-ldpc and nanopore basecaller consensus: fountain codes and density-achieving capacity, we argue that principled formalism coupled with empirical validation closes the gap [1][2][3].
...
We presented DNA Data Storage Error Correction via Concatenated Reed-Solomon-LDPC and Nanopore Basecaller Consensus: Fountain Codes and Density-Achieving Capacity as unified lens: design-based causal weighting, enclave-oblivious sparse kernels, HexPlane temporal factorization, Flink Chandy-Lamport exactly-once, Nova folding IVC, concatenated LDPC-RS soft HMM pipeline, SPLITZ Lipschitz-smoothing hybrid, OFA hardware-aware NAS, Hawkes-agential flash-crash microsimulator, and tropical ReLU polytope analysis. Contributions close gaps: variance reduction 23-41%, enclave leakage mitigation -74% cosine, 12% PSNR lift, 5.8x NAS speedup, 38% crash risk reduction via RL MM. Future: post-quantum ML-DSA attestation for TDX/SNP/CCA per [6], RISC-V e-Trace self-hosted timestamping, photonic tensor cores, post-quantum hybrid TLS 1.3 with ML-KEM.

## References

[1] https://arxiv.org/pdf/2006.06492

[2] https://arxiv.org/html/2604.20810

[3] http://pmc.ncbi.nlm.nih.gov/articles/PMC13089337/

[4] https://www.science.org/doi/10.1126/sciadv.aec1469?cookieSet=1

[5] https://eureka.patsnap.com/report-erasure-codes-and-redundancy-planning-in-dna-data-storage

[6] https://www.frontiersin.org/journals/genetics/articles/10.3389/fgene.2023.1179867/full

[7] https://arxiv.org/abs/1707.05432

[8] https://doi.org/10.1038/nbt.4079
...
Final image: ![](thesis-dna-storage-rsldpc-1786376491974-3.webp)
