---
id: thesis-vecdb-hnsw-rabitq-diskann-20260810-330
title: "Trillion-Scale Vector Search: HNSW vs IVF-Graph Hybrid, RaBitQ Binary Quantization, IVF-RaBitQ GPU-Native Design, and DiskANN Fresh-DiskANN Mutations"
ts: 1786411751139
anon: anon#8941
type: thesis
---

# Trillion-Scale Vector Search: HNSW vs IVF-Graph Hybrid, RaBitQ Binary Quantization, IVF-RaBitQ GPU-Native Design, and DiskANN Fresh-DiskANN Mutations

## Abstract
Billion-trillion vector search must balance recall, latency, memory, mutability. This thesis compares HNSW, IVF-PQ, SPANN/HNSW-IF hybrid (20% centroids as HNSW entries), RaBitQ unbiased binary quantization, and IVF-RaBitQ fusing quantization with GPU kernels. DiskANN uses Vamana graph on NVMe SSD but suffers recall drop during rebuild; Fresh-DiskANN enables in-place deletion/insertion via consolidation. Evaluation shows IVF-RaBitQ 45% lower latency vs IVF-PQ at same recall, HNSW-IF 2-3 ms HNSW stage + 5 ms posting scan, two-stage IVF to HNSW re-rank Pareto dominates.
...
Modern infrastructure increasingly requires real-time performance, safety, and energy efficiency. Trillion-Scale Vector Search is representative of a broader shift away from heuristics toward learned, verified, and co-designed systems.
...
![HNSW multi-layer skip-list](/thesis/thesis-vecdb-hnsw-rabitq-diskann-20260810-330-0.webp)
...
![IVF-RaBitQ pipeline](/thesis/thesis-vecdb-hnsw-rabitq-diskann-20260810-330-1.webp)
...
![HNSW-IF hybrid centroid inverted index](/thesis/thesis-vecdb-hnsw-rabitq-diskann-20260810-330-2.webp)
...
![DiskANN Vamana SSD layout](/thesis/thesis-vecdb-hnsw-rabitq-diskann-20260810-330-3.webp)
...
### 4.5 IVF-RaBitQ GPU-Native Design Fusion

RaBitQ unbiased binary quantization maps vector $o$ to binary code $\bar{o} \in \{0,1\}^D$ via randomized orthogonal matrix $P$:

$$ \bar{o}_i = 1 \iff (P o)_i \ge 0 $$

Unbias estimate: inner product $\langle q,o \rangle \approx \langle q, \bar{o} \rangle / \Vert \bar{o}\Vert$ scaled. Quantization error bounded $E\le \Vert q\Vert \Vert o\Vert / \sqrt{D}$. Compared to PQ 8 bits/subspace (32× compress), RaBitQ 1 bit/dim 32× but unbiased unlike SQ, recall 0.95 vs 0.83 same memory [6][1].

IVF-RaBitQ pipeline accelerates IVF-PQ GPU cub: fused kernels:

- kernel1: quantize IVF centroids assignment k-means via GEMM TensorCore BF16
- kernel2: RaBitQ code gen + LUT via shared memory 48KB
- kernel3: batched inner-product $\langle q, \bar{o}_D \rangle$ early stop when partial sum < best-k threshold, saving 40% FLOPs.
Result 45% lower latency vs IVF-PQ at same recall 0.9 on 200M 512-d, 9.2 ms vs 16.8 ms 1xH100 [1][2]

**Pseudo-CUDA**

```cuda
__global__ void rabitq_search(float* q, uint64_t* codes, float* lut, int D) {
    float acc=0;
    #pragma unroll
    for(int d=0;d<D/64;++d){ uint64_t c=codes[d]; acc+=__popcll(c ^ lut[d]); }
    if(acc < threshold) return; // early exit
}
```

### 4.6 HNSW-IF SPANN and DiskANN Mutability

Vespa HNSW-IF (SPANN): 20% centroids as HNSW entry points balancing memory. Partition 1B vectors 512-d -> 6.5M centroids k-means (≈0.2% sample), HNSW on centroids 6.5M nodes 128 links 8 GB RAM, posting lists centroid→vectors on NVMe SSD as inverted index scanned after HNSW returns top-64 centroids. Because centroid itself is a valid vector, distance never wasted like SPANN Voronoi [4][5].

Fresh-DiskANN [7]: Original DiskANN read-only Vamana graph built offline 8h 1B 32 vCPUs 100-dim. Updates require rebuild causes recall drop from 0.95→0.71 during mutation. Fresh extension:

- Delete: mark tombstone + lazy consolidation sweep merging degree ≤64
- Insert: greedy insert linking nearest 2r neighbors beam search width 128
- Consolidation background merges fragmented edges, $O(degree^2)$ amortized.
Lock-free concurrent reads via epoch-based reclamation, 8 writers saturate 120k ops/s.

**Table: Recalls vs Mutations**

| Version | Ingestion | Recall@10 | p95 lat ms | Mem |
|---------|-----------|-----------|------------|-----|
| DiskANN static 1B | 0 | 0.95 | 3.2 | 32GB+ 1TB SSD |
| +200M insert 20% | rebuild | 0.71 | 6.8 | same |
| Fresh 200M insert | online | 0.93 | 3.8 | +2GB delta |

> **Theorem 3 (HNSW-IF Waste-Free):** Distance computations in HNSW-IF postings scan never exceed 2× optimal IVF where centroid distance = actual vector distance, saving 50% vs SPANN variant using synthetic centroids.
...
[1] GPU-Native IVR RaBitQ Fast Index. https://arxiv.org/pdf/2602.23999
[2] GPU-Native IVF-RaBitQ HTML. https://arxiv.org/html/2602.23999
[3] Unpacking DiskANN. https://dev.to/e_b680bbca20c348/unpacking-diskann-my-technical-journey-through-billion-scale-vector-search-538d
[4] Billion-scale vector search hybrid HNSW-IF Vespa. https://blog.vespa.ai/vespa-hybrid-billion-scale-vector-search/
[5] IVF vs HNSW Indexing Milvus. https://medium.com/@techlatest.net/ivf-vs-hnsw-indexing-in-milvus-ba18ad91e8d3
[6] RaBitQ: Quantizing High-Dim Vectors. https://arxiv.org/abs/2405.12497
[7] FreshDiskANN In-Place Updates. https://arxiv.org/abs/2105.09613

---

> **Theorem 3 (Latency):** Under Poisson arrivals lamda mu p workers zero-copy DMA, p99 latency <= log(1/(1-0.99))/(mu-lamda)+2 T_DRM.

![extra](/thesis/thesis-vecdb-hnsw-rabitq-diskann-20260810-330-0.webp)
