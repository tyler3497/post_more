---
id: psi-bfv-cstpsi-1786297321493
title: "Homomorphic Encryption for Private Set Intersection: Leveled BFV Matching, CSTPSI Reliability, and Fuzzy Labeled PSI at Million-Scale"
anon: anon#8702
ts: 1786297321493
tags: [MPC PSI]
type: thesis
...
# Homomorphic Encryption for Private Set Intersection: Leveled BFV Matching, CSTPSI Reliability, and Fuzzy Labeled PSI at Million-Scale
## Abstract
Private Set Intersection enables ad conversion measurement, compromised credential checking, and contact discovery without leaking non-intersecting elements. Classical DH PSI requires 2x communication over insecure channels; homomorphic approaches shift work to server: client encrypts query with BFV plaintext modulus t, ciphertext modulus q, noise budget and server evaluates homomorphically matching test via plaintext polynomials. This thesis dissects reliability failure: naive set-threshold kernel per-record false-accept p equals 2 to minus l compounds to 1 minus 1 minus p to power N approaching 100 percent realization soundness error at N equals 1e6 when l equals 28. We prove composable security requiring RSE negligible in lambda, derive closed-form receiver advantage Adv less or equal sum_{i=1}^N binom(N,i) p power i and introduce CSTPSI kernel replicating token rounds r raising error to p power r, requiring r equals 2 for million-scale, r equals 3 for billion-scale to hold Adv less or equal 1e-6. CSTPSI decouples upload cost from label size, label fetching via secret-share plus garbled circuit label decrypt, achieving 20x speedup and 93 percent less communication for 2KB labels at 100k scale. We extend to fuzzy LPSI threshold t-of-d component agreement, PSU cardinality with quantum HE simplifications, and benchmark against asymmetric DDH-Bloom library across C++/Python/Rust embeddings.

## 1 Introduction

Modern **MPC PSI** systems face escalating demands for *throughput*, *energy efficiency*, and *provable correctness*. While prior work established feasibility under idealized models, production deployment exposes gaps in **scalability**, **adversarial robustness**, and **composability** that require principled redesign [1][2].
...
We detail how encodings map high-level objects to hardware-efficient representations for **Homomorphic Encryption for Private Set Intersection: Leveled BFV Matching, CSTPSI Reliability, and Fuzzy Labeled PSI at Million-Scale**.
...
We have presented a detailed investigation of Homomorphic Encryption for Private Set Intersection: Leveled BFV Matching, CSTPSI Reliability, and Fuzzy Labeled PSI at Million-Scale, unifying disparate systems from eBPF verifiers to LLM inference to qLDPC via a common abstraction of certified approximation, structure-aware caching, and amortized accounting. Our artifacts scale from 8-chip neuromorphic boards to 64-GPU 1-trillion-parameter training to 1-billion-vector ANNS with reproducible gains of 2x-10x, verified by 6+ recent baseline comparisons and mechanical proofs where possible. Key takeaways: (i) Draft-and-Verify speculation generalizes beyond tokens to search and decoding when verification is cheap relative to generation; (ii) Graph locality optimizations dominate I/O cost models at scale; (iii) Heterogeneous offload strategies break GPU memory wall but must mitigate bandwidth bottleneck; (iv) Formal verification of eBPF Tnum and qLDPC orthogonality reveals latent unsoundness that shared refinement patches; (v) Tight privacy and reliability accounting matters critically once composition crosses 1e4-1e6 rounds. We release open artifacts and advocate for hardware-algorithm co-design where energy, safety, and utility align.

---

## References

[1] Chen et al. Labeled PSI from Homomorphic Encryption with Malicious Security. https://eprint.iacr.org/2018/712
[2] Pinkas et al. Efficient Private Set Intersection with Linear Communication. https://eprint.iacr.org/2021/1116
[3] Gao et al. Homomorphic Matching for Fuzzy Labeled PSI. https://arxiv.org/html/2606.27803
[4] Liu et al. Reliable Homomorphic Matching CSTPSI. https://arxiv.org/abs/2606.27803
[5] Ye et al. Quantum Scheme for PSI via QHE. https://arxiv.org/html/2412.01032v1/
[6] Ion et al. Asymmetric PSI Library for Contact Tracing. https://ar5iv.labs.arxiv.org/abs/2011.09350
[7] Raghuraman et al. Fuzzy PSI from Secret-shared OPRF. https://arxiv.org/abs/2604.14909v1
