---
id: thesis-cert-robustness-smoothing-1786376492974
...
## Abstract
Adversarial vulnerability motivates certified robustness: provable invariance within lp ball. Two strands: randomized smoothing probabilistic averaging under Gaussian noise, and Lipschitz deterministic certificates via 1-Lipschitz networks. Lipschitz methods underfit CIFAR-100/ImageNet with small VGG architectures. We propose SPLITZ splitting network into low-Lipschitz first half and smoothed second half, deriving closed-form certified radius. Wulff Crystal theory yields optimal anisotropic noise PDFs for lp universality improved via central-concentration optimization. Interval Bound Propagation tightened via CROWN-IBP. Experiments: LipNeXt scales to billion parameters achieving 71.3% certified accuracy at epsilon 0.5 l2 on CIFAR-10.

## 1. Introduction

> **Motivation:** Certified Robustness via Randomized Smoothing and Lipschitz Bounded Networks remains central despite decade of progress; unconstrained scaling exposes brittleness under interference, efficiency, and verification.

Modern systems face tension between **expressivity** and **tractability**. In certified robustness via randomized smoothing and lipschitz bounded networks: wulff crystal cascades, interval bound propagation, and conformal guarantees, we argue that principled formalism coupled with empirical validation closes the gap [1][2][3].
...
Final image: ![](thesis-cert-robustness-smoothing-1786376492974-3.webp)
