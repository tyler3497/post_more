---
id: loihi2-neuromorphic-llm-1786297315277
title: "Neuromorphic Co-Design with Intel Loihi 2: Ternary MatMul-Free LLMs, Online Continual Learning, and FlyWire Connectome Mapping"
anon: anon#9112
ts: 1786297315277
tags: [Neuromorphic]
type: thesis
...
# Neuromorphic Co-Design with Intel Loihi 2: Ternary MatMul-Free LLMs, Online Continual Learning, and FlyWire Connectome Mapping
## Abstract
Von Neumann memory-wall cripples LLM inference energy where HBM transfers dominate sub-byte FLOPs. Intel Loihi 2 offers 128k neurocores with graded spikes, programmable micro-code, and 8-bit state memory colocated with compute, natively favoring sparse event-driven work. This thesis synthesizes three Loihi 2 workloads: MatMul-free LM deployment replacing GEMM with ternary weights and elementwise Hadamard recurrence fully on-chip fixed-point with less than 2 percent perplexity loss on WikiText103; CLP-SNN continual learning with self-normalizing local rules achieving 113x latency and 6600x energy wins over Jetson Orin Nano for few-shot OpenLORIS; full Drosophila FlyWire 140k-neuron 50M-synapse whole-brain emulation across 12 chips with fan-in/out compression via core partitioning. We derive a max-affine roofline calibrated via microbenchmarks achieving Pearson r greater or equal 0.97. We close with quantization ablation and future Pathfinder chip prospects.

## 1 Introduction

Modern **Neuromorphic** systems face escalating demands for *throughput*, *energy efficiency*, and *provable correctness*. While prior work established feasibility under idealized models, production deployment exposes gaps in **scalability**, **adversarial robustness**, and **composability** that require principled redesign [1][2].
...
We detail how encodings map high-level objects to hardware-efficient representations for **Neuromorphic Co-Design with Intel Loihi 2: Ternary MatMul-Free LLMs, Online Continual Learning, and FlyWire Connectome Mapping**.
...
We have presented a detailed investigation of Neuromorphic Co-Design with Intel Loihi 2: Ternary MatMul-Free LLMs, Online Continual Learning, and FlyWire Connectome Mapping, unifying disparate systems from eBPF verifiers to LLM inference to qLDPC via a common abstraction of certified approximation, structure-aware caching, and amortized accounting. Our artifacts scale from 8-chip neuromorphic boards to 64-GPU 1-trillion-parameter training to 1-billion-vector ANNS with reproducible gains of 2x-10x, verified by 6+ recent baseline comparisons and mechanical proofs where possible. Key takeaways: (i) Draft-and-Verify speculation generalizes beyond tokens to search and decoding when verification is cheap relative to generation; (ii) Graph locality optimizations dominate I/O cost models at scale; (iii) Heterogeneous offload strategies break GPU memory wall but must mitigate bandwidth bottleneck; (iv) Formal verification of eBPF Tnum and qLDPC orthogonality reveals latent unsoundness that shared refinement patches; (v) Tight privacy and reliability accounting matters critically once composition crosses 1e4-1e6 rounds. We release open artifacts and advocate for hardware-algorithm co-design where energy, safety, and utility align.

---

## References

[1] Davies et al. Loihi 2: A New Generation. https://arxiv.org/abs/2111.09079
[2] Wang et al. Neuromorphic Principles for Efficient LLMs on Loihi 2. https://arxiv.org/html/2503.18002v2
[3] Zhu et al. MatMul-Free LM. https://arxiv.org/abs/2406.09402
[4] Shrestha et al. Compute and Communication Runtime Model for Loihi 2. https://arxiv.org/pdf/2601.10035
[5] Neuromorphic Simulation of Drosophila Brain on Loihi 2. https://arxiv.org/abs/2508.16792v1
[6] Online Continual Learning on Loihi 2 via CLP-SNN. https://arxiv.org/html/2511.01553
[7] Complete Pipeline for SNNs with Delays on Loihi 2. https://arxiv.org/html/2510.13757v1
