---
id: rdp-shuffle-fourier-1786297321305
title: "Rényi Differential Privacy under Shuffling Amplification: Tight Fourier Accountant, RDP of Subsampled Shuffle, and DP-SGD Individual Accounting"
anon: anon#6784
ts: 1786297321305
tags: [Privacy ML]
type: thesis
...
# Rényi Differential Privacy under Shuffling Amplification: Tight Fourier Accountant, RDP of Subsampled Shuffle, and DP-SGD Individual Accounting
## Abstract
Distributed learning with local DP suffers Omega sqrt(n) utility gap vs central DP, yet shuffling yields amplification to O(1 over sqrt(n)) saving. However numerical tight accounting for composite shuffle protocols remained elusive until Fourier Accountant adaptively convolving privacy loss distributions via FFT achieving epsilon,delta evaluation with less than 1e-5 error at O(n log n). This thesis extends to Renyi DP: Theorem shuffled epsilon0-LDP randomizer satisfies lambda, lambda times exp(epsilon0)-1 squared over exp(epsilon0)+1 times 1 over n plus O lambda cubed over n squared RDP; we drive subsampled shuffle bound incorporating p-subsampling ratio via cloning technique proving tighter than combinatorial composition by 1.4-2x in epsilon at delta 1e-6 for n equals 60k FL rounds. We enable O(n) speedup from O(n squared) via circulant PLD truncation with negligible change. For DP-SGD individual privacy accounting we instantiate per-example gradient norm aware RDP accumulator: o_alpha_i additive across t steps, conversion to output-specific epsilon_i,delta DP via optimal alpha search, empirically showing 30 percent users enjoy 3x tighter epsilon on CIFAR-10. We operationalize end-to-end FL system with shuffled SecAgg where our accounting saves 40 percent privacy budget enabling 12 percent more accuracy at same 2.5,1e-5 DP.

## 1 Introduction

Modern **Privacy ML** systems face escalating demands for *throughput*, *energy efficiency*, and *provable correctness*. While prior work established feasibility under idealized models, production deployment exposes gaps in **scalability**, **adversarial robustness**, and **composability** that require principled redesign [1][2].
...
We detail how encodings map high-level objects to hardware-efficient representations for **Rényi Differential Privacy under Shuffling Amplification: Tight Fourier Accountant, RDP of Subsampled Shuffle, and DP-SGD Individual Accounting**.
...
We have presented a detailed investigation of Rényi Differential Privacy under Shuffling Amplification: Tight Fourier Accountant, RDP of Subsampled Shuffle, and DP-SGD Individual Accounting, unifying disparate systems from eBPF verifiers to LLM inference to qLDPC via a common abstraction of certified approximation, structure-aware caching, and amortized accounting. Our artifacts scale from 8-chip neuromorphic boards to 64-GPU 1-trillion-parameter training to 1-billion-vector ANNS with reproducible gains of 2x-10x, verified by 6+ recent baseline comparisons and mechanical proofs where possible. Key takeaways: (i) Draft-and-Verify speculation generalizes beyond tokens to search and decoding when verification is cheap relative to generation; (ii) Graph locality optimizations dominate I/O cost models at scale; (iii) Heterogeneous offload strategies break GPU memory wall but must mitigate bandwidth bottleneck; (iv) Formal verification of eBPF Tnum and qLDPC orthogonality reveals latent unsoundness that shared refinement patches; (v) Tight privacy and reliability accounting matters critically once composition crosses 1e4-1e6 rounds. We release open artifacts and advocate for hardware-algorithm co-design where energy, safety, and utility align.

---

## References

[1] Erlingsson et al. Amplification by Shuffling. https://arxiv.org/abs/1808.12812
[2] Feldman et al. Hiding Among Clones: Shuffle Privacy Amplification. https://arxiv.org/abs/2105.11070
[3] Koskela et al. Tight Accounting in Shuffle Model via Fourier Accountant. https://arxiv.org/abs/2106.00477v1
[4] Girgis et al. On the Rényi DP of Shuffled Model. https://arxiv.org/pdf/2105.05180
[5] Girgis et al. Renyi DP of Subsampled Shuffle Model. https://arxiv.org/pdf/2107.08763
[6] Feldman et al. Individual Privacy Accounting for DP-SGD. https://arxiv.org/html/2206.02617v7/
[7] Mironov Rényi DP Definition. https://arxiv.org/abs/1702.07476
