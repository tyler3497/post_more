---
id: thesis-causal-interference-1786376486974
...
## Abstract
We formalize causal inference under interference where unit outcomes depend on peers via exposure mappings, integrating design-based and model-based perspectives. Classical SUTVA fails in networked A/B tests, requiring Horvitz-Thompson and Hájek inverse-propensity weighting over joint treatment-exposure probabilities. We derive unbiasedness and variance bounds under Bernoulli and cluster-randomized designs, analyze positivity violations for isolated nodes, and introduce doubly-robust augmented IPW estimators with machine-learning nuisance models via three-fold cross-fitting for edge-level outcomes. Theoretical results show central limit theorems under limited dependence, with conservative variance estimators that avoid over-conservatism by exploiting identifiable network correlation structure. Empirical evaluation on Add Health and simulated Watts-Strogatz graphs demonstrates 23-41% variance reduction and valid 95% coverage under spillover. We discuss implications for platform experiments and interference-aware policy learning.

## 1. Introduction

> **Motivation:** Causal Inference under Interference remains central despite decade of progress; unconstrained scaling exposes brittleness under interference, efficiency, and verification.

Modern systems face tension between **expressivity** and **tractability**. In causal inference under interference: horvitz-thompson estimators, cluster-randomized designs, and double robustness for networked a/b testing, we argue that principled formalism coupled with empirical validation closes the gap [1][2][3].
...
Final image: ![](thesis-causal-interference-1786376486974-3.webp)
