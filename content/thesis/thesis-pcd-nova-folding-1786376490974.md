---
id: thesis-pcd-nova-folding-1786376490974
...
## Abstract
Recursive SNARKs enable incrementally verifiable computation. Nova introduces folding reducing two relaxed R1CS instances to one via single group scalar multiplications, avoiding SNARK in recursion overhead. HyperNova generalizes to CCS, SuperNova to non-uniform NIVC for ZKVM opcode dispatch. We formalize IVC via PCD: prover folds (U,W) and (u,w) to (U',W',T), then compresses via Spartan/Mercury/HyperKZG over Pallas/Vesta cycles. Custom gates for VM state use Pedersen commitments; binary tree folding parallelizes 4x. Implementation nova-snark in Rust achieves prover 2 multiexps per step, verifier constant.

## 1. Introduction

> **Motivation:** Proof-Carrying Data for Recursive zk-SNARK Composition remains central despite decade of progress; unconstrained scaling exposes brittleness under interference, efficiency, and verification.

Modern systems face tension between **expressivity** and **tractability**. In proof-carrying data for recursive zk-snark composition: nova folding, supernova non-uniform ivc, and custom gates for incrementally verifiable computation, we argue that principled formalism coupled with empirical validation closes the gap [1][2][3].
...
We presented Proof-Carrying Data for Recursive zk-SNARK Composition: Nova Folding, SuperNova Non-Uniform IVC, and Custom Gates for Incrementally Verifiable Computation as unified lens: design-based causal weighting, enclave-oblivious sparse kernels, HexPlane temporal factorization, Flink Chandy-Lamport exactly-once, Nova folding IVC, concatenated LDPC-RS soft HMM pipeline, SPLITZ Lipschitz-smoothing hybrid, OFA hardware-aware NAS, Hawkes-agential flash-crash microsimulator, and tropical ReLU polytope analysis. Contributions close gaps: variance reduction 23-41%, enclave leakage mitigation -74% cosine, 12% PSNR lift, 5.8x NAS speedup, 38% crash risk reduction via RL MM. Future: post-quantum ML-DSA attestation for TDX/SNP/CCA per [6], RISC-V e-Trace self-hosted timestamping, photonic tensor cores, post-quantum hybrid TLS 1.3 with ML-KEM.

## References

[1] https://crypto.iacr.org/2022/papers/538806_1_En_13_Chapter_OnlinePDF.pdf

[2] http://www.contrib.andrew.cmu.edu/~bparno/papers/hypernova.pdf

[3] https://github.com/microsoft/Nova/blob/main/README.md

[4] https://hackmd.io/@oskarth/HkcWyHK-2

[5] https://github.com/crazy-cryptographic-buddies/presentation-nova-recursive-zero-knowledge-arguments-from-folding-scheme

[6] https://eprint.iacr.org/2025/1653.pdf

[7] https://blog.lambdaclass.com/incrementally-verifiable-computation-nova/

[8] https://arxiv.org/abs/2203.08185
...
Final image: ![](thesis-pcd-nova-folding-1786376490974-3.webp)
