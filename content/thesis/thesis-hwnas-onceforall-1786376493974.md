---
id: thesis-hwnas-onceforall-1786376493974
...
## Abstract
Edge inference under 10ms on ARM/DSP requires hardware-aware NAS. OFA trains one supernetwork with progressive shrinking across depth, width, kernel, resolution yielding 1e19 subnetworks eliminating retrain per deployment. We improve latency estimation replacing lookup tables with generalized neural-network-twins predictors using hardware features achieving 50% lower RMSE vs ProxylessNAS. GPU-generalization and fine-tuning transfer enable 5.8x NAS speedup. Deployment search uses evolutionary NSGA-II exploring accuracy-latency Pareto with sub 0.5 GPUh. Quantization-aware finetuning to INT4 yields 2.6x latency vs EfficientNet at same ImageNet top-1 80.0% under 600M MACs mobile.

## 1. Introduction

> **Motivation:** Hardware-Aware Neural Architecture Search for Sub-10ms Edge Inference remains central despite decade of progress; unconstrained scaling exposes brittleness under interference, efficiency, and verification.

Modern systems face tension between **expressivity** and **tractability**. In hardware-aware neural architecture search for sub-10ms edge inference: once-for-all supernetworks, pred-nas latency predictors, and int4 quantization-aware finetuning, we argue that principled formalism coupled with empirical validation closes the gap [1][2][3].
...
Final image: ![](thesis-hwnas-onceforall-1786376493974-3.webp)
