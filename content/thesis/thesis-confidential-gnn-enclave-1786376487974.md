---
id: thesis-confidential-gnn-enclave-1786376487974
...
## Abstract
Confidential graph neural network training on sensitive topology requires TEEs. We analyze privacy leakage via gradient leakage attacks on GraphSAGE, GCN, GIN and GAT showing attention exacerbates leakage while injective aggregation mitigates. We propose PRIVADO-GNN: input-oblivious sparse aggregation using fixed-access ORAM, vertically federated GraphSAGE with homomorphic embedding aggregation, and distributed trusted execution with oblivious fixed-rate communication to block side-channels. Micro-benchmarks: oblivious aggregation adds 17.2% overhead, SGX enclave inference latency reduction 8.8-14.2x vs naive encrypted baselines via graph-aware sparse elimination. Security proof reduces to remote attestation.

## 1. Introduction

> **Motivation:** Secure Enclaves for Confidential GNN Training remains central despite decade of progress; unconstrained scaling exposes brittleness under interference, efficiency, and verification.

Modern systems face tension between **expressivity** and **tractability**. In secure enclaves for confidential gnn training: oblivious aggregation, graphsage federated partitioning, and side-channel hardened sparse kernels, we argue that principled formalism coupled with empirical validation closes the gap [1][2][3].
...
Final image: ![](thesis-confidential-gnn-enclave-1786376487974-3.webp)
